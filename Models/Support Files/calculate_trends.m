...
for i=1:length(vec)-1
    ...
    % Pext
    Pext(i) = PeLBn(vec(i)); % Creates new external pressure list that is the same length as other outputs
    ...
    %HR
    HRnew(i) = HR(vec(i)); % Creates new HR list that is the same length as other outputs

    %CO
    CO(i)=(sum(Qlo(st:en-1).*step(st:en-1))/(tout(en)-tout(st)))*(6/100); %l/min

    % Stroke Volume
    SV(i) = max(Vlv(st:en-1))- min(Vlv(st:en-1)); %Stroke Volume from change in ventricular volume
    SVV(i)= (sum(Qlo(st:en-1).*step(st:en-1))/(tout(en)-tout(st)))*((tout(en)-tout(st))); %Stroke volume from integration of out flow

    %CO
    %COnew(i) = ((HRnew(i))/60).*SV(i)/100; % L/min
